...
"@openzeppelin/contracts/access/AccessControl.sol"; // This
allows
role - based
access
control
...
"@openzeppelin/contracts/token/ERC20/ERC20.sol"; // This
contract
needs
to
interact
...
contract
AMM is AccessControl
{
    bytes32
public
constant
LP_ROLE = keccak256("LP_ROLE");
uint256
public
invariant;
address
...
qtyA = (10000 - feebps) / 10000 * invariant / qtyB;
...
invariant = ERC20(tokenA).balanceOf(address(this)) * ERC20(tokenB).balanceOf(address(this));
emit
LiquidityProvision(msg.sender, amtA, amtB);
}
...
invariant = ERC20(tokenA).balanceOf(address(this)) * ERC20(tokenB).balanceOf(address(this));
emit
Withdrawal(msg.sender, recipient, amtA, amtB);
}
